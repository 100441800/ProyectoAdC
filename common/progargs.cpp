...
void check_inputs(const std::vector<std::string>& inputs){
	...
	if(!fs::exists(in_path)) {
		throw std::invalid_argument("Input directory [" + inputs[0] + "] does not exist\n" + INSTRUCTIONS);
	} else if(!fs::exists(out_path)) {
		throw std::invalid_argument("Output directory [" + inputs[1] + "] does not exist\n" + INSTRUCTIONS);
	}
	// Read and write permissions
	if((fs::status(inputs[0]).permissions() & fs::perms::owner_read) == fs::perms::none) {
		throw std::invalid_argument("Cannot open directory [" + inputs[0] + "]\n" + INSTRUCTIONS);
	} else if ((fs::status(inputs[1]).permissions() & fs::perms::owner_write) == fs::perms::none) {
